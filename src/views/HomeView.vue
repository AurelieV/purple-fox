<template>
  <main class="text-sm md:text-md">
    <section class="relative aspect-square sm:aspect-[3/1] xl:aspect-[4/1] hero-section">
      <div
        class="absolute py-2 top-[5%] left-[5%] md:top-[30%] md:left-[10%] md:py-4 rounded w-[12ch] text-center text-xl sm:text-2xl md:text-3xl lg:text-6xl font-bold text-white backdrop-blur-sm"
      >
  ...
  <footer class="px-2 pt-4 pb-3 text-xs text-center text-white font-italic">
    ...
    <p>
      ...
      <a href="patreon.com/PurpleFoxSoftwares">Patreon</a> :)
    </p>
    <p>For more information, please join the <a href="https://discord.gg/rSRcxMySEF">Discord server</a>.</p>
  </footer>
</template>
...
const sections = [
  {
    title: 'Navigate the End of Round',
    description:
...
<style scoped>
.hero-section {
  background-image: url('/src/assets/home_background1_1.png');
...
@media screen(sm) {
  .hero-section {
    background-image: url('/src/assets/home_background3_1.png');
  }
}
@media screen(xl) {
  .hero-section {
    background-image: url('/src/assets/home_background4_1.png');
  }
}
...
#under-hero {
  box-shadow:
    0px 4px 0px 0px #1b0c51,
    0px 12px 20px 0px rgba(27, 12, 81, 0.8);
}
footer a {
  @apply underline hover:text-purple;
}
...
.decorate::after {
  ...
  height: var(--height);
  position: absolute;
  bottom: calc(var(--height) * -1 / 2);
  left: calc(50% - var(--width) / 2);
  @apply mx-auto bg-pink;
}
</style>
